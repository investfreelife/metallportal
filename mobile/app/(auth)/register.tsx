...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    if (!fullName || !email || !password) { Alert.alert('Ошибка', 'Заполните все поля'); return; }
    setLoading(true);
    const { data, error } = await supabase.auth.signUp({ email, password });
    if (error) { setLoading(false); Alert.alert('Ошибка', error.message); return; }
    if (data.user) {
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled">
        <View style={s.inner}>
          ...
          <View style={s.fieldWrap}>
            ...
            <TextInput style={s.input} placeholder="ООО Металл Сервис" value={fullName} onChangeText={setFullName} />
          </View>
          <View style={s.fieldWrap}>
          ...
          <View style={s.fieldWrapLast}>
            ...
            <TextInput style={s.input} placeholder="Минимум 6 символов" value={password}
          ...
          <View style={s.footer}>
            ...
            <Link href="/(auth)/login"><Text style={s.link}>Войти</Text></Link>
          </View>
        </View>
      </ScrollView>
...
const s = StyleSheet.create({
  ...
  roleBtn: { flex: 1, paddingVertical: 12, borderRadius: 12, borderWidth: 1, borderColor: '#cbd5e1', alignItems: 'center' },
