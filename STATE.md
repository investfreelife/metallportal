...
**Кто:** Сергей
**Что:** (1) Открыть `/privacy` и `/oferta` — заполнить `[ИНН]`, `[ОГРН]`, `[р/с]`, `[Банк]` реальными реквизитами. (2) Подать уведомление в Роскомнадзор (ст.22 152-ФЗ) или зафиксировать основание для исключения. (3) Проверить DPA с OpenRouter или убедиться что PII туда не уходит. (4) Заполнить `docs/supplier-selection-checklist.md`. (5) Дать сигнал «Week 1 готова».
**Когда готово:** После пп. 1-3 → 5 из 8 152-ФЗ блокеров закрыто. Week 1 можно завершать.
