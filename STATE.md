...
2026-04-26  ✅  validate_pricing() интегрирован в tasks.py::generate_kp + @with_validation на search_metal
...
```
- 2026-04-27: [БЛОКЕР] 152-ФЗ pre-flight — 0/8 пунктов закрыто. Кто разблокирует: Сергей. Срок: до конца Week 1.
  [ ] Оферта на сайте опубликована (B2B + B2C версии)
  [ ] Политика обработки ПДн (152-ФЗ) на /privacy
  [ ] Согласие на обработку ПДн в формах SmartSearch + checkout (чекбокс с ссылкой)
  [ ] Уведомление в РКН подано (или зафиксировано основание по ст.22 ч.2)
  [ ] PII логируется только в платных моделях — проверить флаг pii=True в tasks.py::_llm()
  [ ] OpenRouter — DPA подписан или PII туда не уходит
  [ ] Cookie-баннер с согласием (Я.Метрика + PostHog)
  [ ] Договор оферты на металлопрокат (B2C: ст.437 ГК; B2B: рамочный)
```
