...
function buildScenarioScript(opts: {
  ...
  const { forwardTo, ownCli, webhookUrl, webhookSecret } = opts
  ...
  // Start recording когда outbound отвечает (обе стороны bridged)
  ...
  // Capture record URL when available (Voximplant populates после Disconnected)
  ...
  e.call.addEventListener(CallEvents.Disconnected, function(event) {
    ...
    var params = [
      ...
      'forwarded_to=' + encodeURIComponent('${forwardTo}'),
...
async function main() {
  ...
  // c024: webhook config — required for recording + call ingestion.
