/**
 * Seed для Шестигранник конструкционный г/к (W2-20).
 *
 * Source: scripts/data/shestigrannik_konstruktsionnyy_skus.json (61 SKUs).
 *
 * Pipeline через lib/raw-import-reconcile.ts (POLICY compliance).
 *
 * Usage:
 *   npx tsx scripts/seed_shestigrannik_konstruktsionnyy.ts             # dry-run
 *   npx tsx scripts/seed_shestigrannik_konstruktsionnyy.ts --commit
 */
...
const CATEGORY_ID = "166f018f-738a-4130-9350-275e05d63981"; // shestigrannik-konstruktsionnyy
const SUPPLIER_ID = "d1000000-0000-0000-0000-000000000001";
const MARKUP_PCT = 9.0;
const CURRENCY = "RUB";

type ParsedShestigr = ParsedSku & {
...
function loadSkus(): ParsedShestigr[] {
  const path = resolve(__dirname, "data", "shestigrannik_konstruktsionnyy_skus.json");
  return JSON.parse(readFileSync(path, "utf-8")) as ParsedShestigr[];
}

async function main() {
  const isCommit = process.argv.includes("--commit");
  const mode = isCommit ? "COMMIT" : "DRY-RUN";
  console.log(`\n=== seed_shestigrannik_konstruktsionnyy.ts — ${mode} ===\n`);
  ...
  const reportPath = resolve(__dirname, "data", "shestigrannik_konstruktsionnyy_reconcile.md");
  ...
  if (!isCommit) {
    for (const s of (result.new as ParsedShestigr[]).slice(0, 5)) {
      console.log(`  ${s.slug.padEnd(40)} | ${s.name.padEnd(50)} | ${s.prices[0].base_price} ₽/т`);
    }
    console.log("\nDry-run complete.");
    return;
  }
  ...
  for (const sku of result.new as ParsedShestigr[]) {
    const productRow = {
      name: sku.name,
      slug: sku.slug,
