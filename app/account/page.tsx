  ...
  useEffect(() => {
    async function load() {
      ...
      if (meData.user) {
        ...
        // Also load site_users referral data (ТЗ #049: stats + 3-level tree)
        try {
          const r2 = await fetch('/api/auth/me');
          const d2 = await r2.json();
          if (d2.user) {
            setSiteUser(d2.user);
            setReferrals(d2.referrals || []);
            setReferralStats(d2.referral_stats || null);
            setReferralTree(d2.referral_tree || null);
          }
        } catch {}
        setLoading(false);
        return;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container-main py-10 max-w-2xl mx-auto space-y-5">
        ...
        {/* Referral tab — ТЗ #049: 3-level MLM, no more bronze/silver/gold */}
        {tab === 'referral' && (
          <div className="space-y-4">
            {!siteUser ? (
              <div className="bg-card border border-border rounded-xl p-6 text-center">
                <p className="text-muted-foreground text-sm mb-3">
                  Войдите по email, чтобы участвовать в реферальной программе и получить персональную ссылку.
                </p>
                <Link href="/account/login?tab=register&mode=email" className="bg-gold text-black px-5 py-2 rounded-xl text-sm font-semibold hover:bg-yellow-400 transition-all">
                  Зарегистрироваться →
                </Link>
              </div>
            ) : referralStats && referralTree ? (
              <ReferralDashboard user={siteUser} stats={referralStats} tree={referralTree} />
            ) : (
              <div className="text-muted-foreground text-sm">Загрузка...</div>
            )}
          </div>
